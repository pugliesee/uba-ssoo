...
LT_END_SUITE(TestsCorpus)

LT_BEGIN_TEST(TestsCorpus, CargarCorpusUnicoThread)
    cargarArchivo(hM, "data/corpus/corpus");
    LT_CHECK_EQ(hM.valor("ginebra"), 7);
    LT_CHECK_EQ(hM.claves().size(), 2247);
LT_END_TEST(CargarCorpusUnicoThread)

LT_BEGIN_TEST(TestsCorpus, CargarCorpusMultiplesThreads)
    cargarMultiplesArchivos(hM, 10, get_paths_to_files(std::filesystem::path{"./data/corpus/split_corpus/"}));
    LT_CHECK_EQ(hM.valor("ginebra"), 1);
    LT_CHECK_EQ(hM.claves().size(), 2247);
LT_END_TEST(CargarCorpusMultiplesThreads)
