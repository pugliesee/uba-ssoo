...
class ListaAtomica {
 ...
 public:
    ...
    void insertar(const T &valor) {
        ...
        // Tomo posesion del recurso para insertar valor
        _mux_insert.lock();

        // Creo un nuevo nodo y lo agrego al principio de la lista
        Nodo *new_node = new Nodo(valor);

        new_node->_siguiente = _cabeza.load(); 
        new_node->_valor = valor;
        _cabeza.store(new_node);
